...
class ITello(metaclass=ABCMeta):
  @staticmethod
  @abstractmethod
...
class Tello(ITello):
    ...
    RESPONSE_TIMEOUT = 14  # in seconds
    TIME_BTW_COMMANDS = 0.1  # in seconds
    TIME_BTW_RC_CONTROL_COMMANDS = 0.001  # in seconds
    RETRY_COUNT = 1  # number of retries after a failed command. Default was 3
    ...
    def __init__(self,
        ...
        self._observers = set() # observer pattern
    ...
    def send_control_command(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> bool:
        ...
        for i in range(0, self.retry_count):
            ...
            if response == 'OK' or response == 'ok':
                if(command == 'command'):
                    self.notify('drone_connected')
                return True
            if(command == 'command'):
                if(response == 'Timeout error!'):
                    self.notify('drone_not_connected')
