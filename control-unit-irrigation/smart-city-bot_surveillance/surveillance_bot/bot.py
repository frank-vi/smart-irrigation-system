...
class Bot:
    ...
    def __init__(
        ...
        if not token:
            ...
            sys.exit(1)
        if not usernames[0] and not usernames[1]:
            self.logger.critical(
                "Error! Missing AUTHORIZED_USERS configuration"
            )
            sys.exit(1)
        ...
        self.authorized_users = usernames
    ...
    def command_handler(
        ...
        def wrapped(update: Update, context: CallbackContext) -> Any:
            ...
            if update.effective_chat.username not in self.authorized_users:
                logger.warning(
        ...
        for data in self.camera.surveillance_start(
            ...
            if 'detected' in data:
                ...
                waiting_message = update.message.reply_text(
                    ...
                    text=f'taking {video_seconds // picture_interval} '
                         f'photos...'
                )
            if 'photo' in data:
                context.bot.send_chat_action(
